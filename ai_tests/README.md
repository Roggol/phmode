...
| `scenarios/move_scoring.c` | Move-scoring rules that depend on speed/HP, e.g. Thunder Wave's strong bonus specifically when the AI is slower than its target; Sucker Punch's bonus when the target's last move was a damaging one; and the new Electric-type paralysis immunity, Grass-type powder-move immunity, and Ghost-type trapping immunity. |
