...
// Set by Space Warp (Palkia): gravity that lasts the whole battle and never
// counts down.
#define FIELD_CONDITION_GRAVITY_PERM           (1 << 23)
// Set by overworld Trick Room (a map's .weather): the dimensions stay twisted
// for the whole battle and never count down. Cleared only by the Trick Room move
// (which toggles it off) and, like the counter, never by weather.
#define FIELD_CONDITION_TRICK_ROOM_PERM        (1 << 24)
...
// The turn counter occupies bits 12-14; GRAVITY_PERM (bit 23) is a separate flag
// for battle-long gravity. Every "is gravity active" check uses FIELD_CONDITION_GRAVITY
// and so honours both; only the per-turn countdown looks at the counter alone.
#define FIELD_CONDITION_GRAVITY_COUNTER (FIELD_CONDITION_GRAVITY_0 | FIELD_CONDITION_GRAVITY_1 | FIELD_CONDITION_GRAVITY_2)
#define FIELD_CONDITION_GRAVITY         (FIELD_CONDITION_GRAVITY_COUNTER | FIELD_CONDITION_GRAVITY_PERM)
#define FIELD_CONDITION_GRAVITY_INIT    (FIELD_CONDITION_GRAVITY_0 | FIELD_CONDITION_GRAVITY_2)
// The turn counter occupies bits 16-18; TRICK_ROOM_PERM (bit 24) is a separate
// battle-long flag. "Is Trick Room active" checks use FIELD_CONDITION_TRICK_ROOM
// and honour both; only the per-turn countdown looks at the counter alone.
#define FIELD_CONDITION_TRICK_ROOM_COUNTER (FIELD_CONDITION_TRICK_ROOM_0 | FIELD_CONDITION_TRICK_ROOM_1 | FIELD_CONDITION_TRICK_ROOM_2)
#define FIELD_CONDITION_TRICK_ROOM      (FIELD_CONDITION_TRICK_ROOM_COUNTER | FIELD_CONDITION_TRICK_ROOM_PERM)
